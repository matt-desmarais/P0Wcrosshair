...
def pattern1( arr, width, height, x, y, rad, col ):
    cv2.line(arr,(0,y),(width,y),col,5)
    cv2.line(arr,(x,0),(x,height),col,5)
    i = 0
    for i in range(1, 8): 
        cv2.circle(arr,(x,y),i*rad,col,5)
        i += 1
    ...
    for i in intervalh:
        ...
        if j%5 == 0:    
            if not j%10 == 0:
                cv2.line(arr,(x+diff,y-4),(x+diff,y+4),col,5)
                cv2.line(arr,(x-diff,y-4),(x-diff,y+4),col,5)
        else:
            cv2.line(arr,(x+diff,y-2),(x+diff,y+3),col,5)
            cv2.line(arr,(x-diff,y-2),(x-diff,y+3),col,5)
        j += 1
    ...
    for k in intervalv:
        ...
        if l%5 == 0:    
            if l%10 == 0:
                cv2.line(arr,(x-6,y+diff),(x+6,y+diff),col,5)
                cv2.line(arr,(x-6,y-diff),(x+6,y-diff),col,5)
            else:
                cv2.line(arr,(x-4,y+diff),(x+4,y+diff),col,5)
                cv2.line(arr,(x-4,y-diff),(x+4,y-diff),col,5)
        else:
            cv2.line(arr,(x-2,y+diff),(x+2,y+diff),col,5)
            cv2.line(arr,(x-2,y-diff),(x+2,y-diff),col,5)
        l += 1
    return    
...
def pattern2( arr, width, height, x, y, rad, col ):
    ...
    for i in intervalh:
        ...
        if j%5 == 0:    
            if j%10 == 0:
                cv2.line(arr,(x+diff,y-6),(x+diff,y+6),col,5)
                cv2.line(arr,(x-diff,y-6),(x-diff,y+6),col,5)
            else:
                cv2.line(arr,(x+diff,y-4),(x+diff,y+4),col,5)
                cv2.line(arr,(x-diff,y-4),(x-diff,y+4),col,5)
...
def pattern7( arr, width, height, x, y, rad, col ):
    cv2.line(arr,(x-10,y),(x+10,y),col,5)
    cv2.line(arr,(x,y-10),(x,y+10),col,5)
    return

# pattern8: small center crosshair without center
def pattern8( arr, width, height, x, y, rad, col ):
    cv2.line(arr,(x-10,y),(x-3,y),col,5)
    cv2.line(arr,(x,y-10),(x,y-3),col,5)
    cv2.line(arr,(x+3,y),(x+10,y),col,5)
    cv2.line(arr,(x,y+3),(x,y+10),col,5)
    return
...
def pattern10( arr, width, height, x, y, rad, col ):
    ...
    # horizontal lines:
    while i < height:
        cv2.line(arr,(0,y+i),(width,y+i),col,5)
        cv2.line(arr,(0,y-i),(width,y-i),col,5)
        i += rad
    # vertical lines:
    while j < width:
        cv2.line(arr,(x+j,0),(x+j,height),col,5)
        cv2.line(arr,(x-j,0),(x-j,height),col,5)
        j += rad
    return
